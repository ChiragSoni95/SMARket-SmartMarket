...
<script type="text/javascript">
    function Validate() {
        var ddl = document.getElementById("brand");
        if (ddl.value == "") {
...
SELECT * from decem where category='<%=request.getAttribute("category")%>' group by brand;
</sql:query>
	<br>
	<br>
	<div class="panel-body">
		<div class="form">
			...
			<form class="form-validate form-horizontal" id="feedback_form"
				...
				<input type="hidden" name="action" value="classify4">
...
<center>
				...
				<select name="brand" id="brand">
					...
					<c:forEach var="row" items="${result.rows}">
						...
						<option value="<c:out value="${row.brand}"/>"><c:out
								value="${row.brand}" /></option>
					</c:forEach>
				</select>
				</h3>
					<br>
	...
	<sql:query dataSource="${ds}" var="result">
SELECT * from decem where category='<%=request.getAttribute("category")%>';
</sql:query>
