			  ...
			  <div class="row">
				<div class="col-lg-12">
					...
					<ol class="breadcrumb">
						...
						<li><i class="fa fa-laptop"></i>Re-order Existing Products</li>						  	
					</ol>
				</div>
			</div>
...
<h1>Left Quantity of Existing Products</h1>

<sql:query dataSource="${ds}" var="result">
SELECT * from decem group by category;
</sql:query>
	<br>
	<br>
	<div class="panel-body">
		<div class="form">
			...
			<form class="form-validate form-horizontal" id="feedback_form"
				...
				<input type="hidden" name="action" value="existing_inventory1">
...
<center>
			...
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			&nbsp;&nbsp;
			Buffer Percentage:&nbsp;&nbsp; &nbsp;&nbsp;<input name="buffer"
							   type="number" style="width:300px;" min="0" max="100" required />
	...
	<sql:query dataSource="${ds}" var="result">
SELECT * from buffer where category='<%=request.getAttribute("category")%>' AND sold_percent>100-<%=request.getAttribute("buffer")%> order by stock_left;
</sql:query>
	...
	<table border="1" width="90%">
		...
		<c:forEach var="row" items="${result.rows}">
			<tr>
				...
				<td><c:out value="${row.stock_left}" /></td>
				<td><a class="btn btn-success" href="<%=request.getContextPath() %>/Controller?action=inventory_calc&product_id=<c:out value="${row.product_id}"/>&category=<c:out value="${row.category}"/>&brand=<c:out value="${row.brand}"/>&sub_category=<c:out value="${row.sub_category}"/>&rate_demand=<c:out value="${row.stock_sold}"/>&cost=<c:out value="${row.cost_price}"/>&holding_cost=<c:out value="${row.storage_price/row.stock_available}"/>" title="Bootstrap 3 themes generator">Re-order</a></td>
